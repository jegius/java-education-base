...
Находясь на этой ветке в меню гит выбрать пункт `rebase current onteo selecte` у ветки в которую будет отправлен 
...
все изменения в твоей ветке на сервере и зальет на него твою текущую ветку. 

**! Использовать Force push можно только на своей ветке и только при полной уверенности что на серевере нет данных которой
нет на твоем текущем пк !**
...
два разных пользователя сделли два разных изменения, то при попытки обновить ветку (например с помощью rebase) 
